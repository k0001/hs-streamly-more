...
decodeStream
  ...
  => S.Stream m BS.ByteString
  -> S.Stream m a
decodeStream = decodeStreamGet Bin.get

decodeStreamGet
  :: forall a m
  .  (Monad m, Ex.MonadThrow m)
  => Bin.Get a
  -> S.Stream m BS.ByteString
  -> S.Stream m a
decodeStreamGet ga
...
decodeStream'
  ...
  => Bin.Get a
  -> S.Stream m BS.ByteString
  -> S.Stream (ExceptT (S.Stream m BS.ByteString, Error) m)
              (Natural, a)
decodeStream' ga = curry g 0 . SK.fromStream
  where
    g :: (Natural, SK.StreamK m BS.ByteString)
      -> S.Stream (ExceptT (S.Stream m BS.ByteString, Error) m)
                  (Natural, a)
...
-- 'Bin.Decoder'.
type Result n a =
  Either (S.Stream n BS.ByteString, Bin.ByteOffset, String)
